...
v = @(C,X,K) C(1)*(C(2)*X(:,1)+X(:,1).^2+C(2)*X(:,1).*X(:,2)/K(1))./...
    (C(2)^2+2*C(2)*X(:,1)+X(:,1).^2+2*C(2)*X(:,1).*X(:,2)/K(1)+2*C(2)^2*...
    X(:,2)/K(1)+C(2)^2*(X(:,2)/K(1)).^2/K(2));

% Equation for estimating IC50
f = @(alpha, IC50, d, I) d - d./(1+(I/IC50).^alpha);

% IC50 equation
H = @(Ki, c, S, Ks) sqrt(c)*Ki*(1+S/Ks);
...
% True values for objective parameters
Kic = 1; c = 0.04;
K = [Kic c];
%% Generate %Inhibition data and estimate IC50
% Substrate concentration for estimating IC50
St_IC50 = Ks;

It_ranged = logspace(-3,3,10);
...
%% Make in silico data with replicates
...
% Raw data: 1st column - Substrate, 2nd column - Inhibitor, 3rd column - V0
data = [raw_X raw_data];